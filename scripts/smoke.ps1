...
try {
  ...
  $login = Invoke-RestMethod -Method Post -ContentType "application/json" -WebSession $session -Uri "http://127.0.0.1:$adminPort/admin/v1/auth/login" -Body (@{ token = $adminToken } | ConvertTo-Json)
  if (-not $login.ok) {
